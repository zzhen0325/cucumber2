...
- 底部输入器居中悬浮，宽度为 `min(600px, calc(100vw - 48px))`；输入器复用 AI Elements Prompt Input 的 Header / Body / Footer slot，Agent 模式约 168px，图像模式约 185px，并在输入器顶部提供 Agent / 图像模式切换。
...
- Run node：淡黄绿色卡片，Agent 对话是主内容；模型 reasoning summary 用 AI Elements Reasoning 折叠展示；计划进度、handoff/skill 摘要和工具调用用 AI Elements Chain of Thought 样式作为 Agent 执行流附属展示，图片等产物由结果节点承载，不在 Run 卡里作为主摘要重复展示。默认保持紧凑，详细参数放在可展开工具行。
