...
- Run node：淡黄绿色卡片，Agent 对话是主内容；计划进度、handoff/skill 摘要和工具调用用 AI Elements Chain of Thought 样式作为 Agent 执行流附属展示，图片等产物由结果节点承载，不在 Run 卡里作为主摘要重复展示。默认保持紧凑，详细参数放在可展开工具行。
