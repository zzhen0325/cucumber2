...
- 稳定、重复使用的颜色、尺寸、圆角和阴影统一维护在 `src/index.css` 的 `--cuc-*` token 中。
- `src/App.css` 只保留组件结构、布局和局部一次性状态值；新增重复样式先补 token，再引用 token。
- token 命名参考 Geist 的分层格式，但数值以当前 Cucumber/Flowith 画布视觉为准，不直接套用 Geist 数值。
