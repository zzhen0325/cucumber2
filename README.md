...
- 通用对话边界：寒暄和简单短答优先走 fast path 或轻量 chat；落到 Manager route 的短问答、概念解释、轻量分析、提示词/文本改写和简短总结直接在 Run 节点回复。用户明确要求详细说明、完整规划、长篇方案、调研分析、报告、文档、模板、提示词模板、可复制/直接使用方案、设定稿或规范时，归一化为 document/markdown artifact 并交给 Document Agent 产出可沉淀的画布文档。
