...
图片生成默认使用 Seedream，需要 `IMAGE_PROVIDER=seedream`、`SEEDREAM_ACCESS_KEY_ID` 和 `SEEDREAM_SECRET_ACCESS_KEY`。也可以使用 Coze：`IMAGE_PROVIDER=coze`、`COZE_IMAGE_TOKEN`，可选 `COZE_IMAGE_URL`、`COZE_IMAGE_SIZE`、`COZE_IMAGE_WATERMARK`、`COZE_IMAGE_MODEL`。Coze 请求体保持 `prompt`、`reference_images`、`size`、`watermark`、`model` 结构；画布上游图片会以 URL 列表写入 `reference_images`。项目、Trace 和对象存储 metadata 持久化需要 `SUPABASE_URL` 与 `SUPABASE_SECRET_KEY`。
