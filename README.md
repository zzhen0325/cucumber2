...
- 启动 Fast Path：路由分三层执行。`Quick Router` 只处理确定性 preflight：寒暄、图片生成元信息直返、简单 canvas 操作和已显式归一化的图像模式输入；其余请求进入 `Fast Intent Router`，由一个 `maxTurns=1` 的结构化 Agents SDK agent 读取当前 agent/tool capability manifest，快速输出 `operation + artifact + domain + requiredCapabilities + negativeCapabilities` 候选。高置信候选直接进入运行；低置信、能力冲突或复杂上下文编辑再进入完整 Input Normalizer。
...
- 通用对话边界：寒暄和简单短答优先走 fast path 或轻量 chat；落到 Manager route 的短问答、概念解释、轻量分析、提示词/文本改写和简短总结直接在 Run 节点回复。用户明确要求详细说明、完整规划、长篇方案、调研分析、报告或文档时，归一化为 document/markdown artifact 并交给 Document Agent 产出可沉淀的画布文档。
...
- Trace：`run.created` / `input.normalized` 记录服务端重建的 selected nodes、reference nodes、upstream path、omitted nodes、route、routerSource、skippedSteps、fast route confidence、preferredRoute、fallbackReason 和 candidateTools；启动阶段会用 `run.step.*` 记录实际执行的上下文重建、快速路由、输入归一化、计划生成、技能检索和 Agent Runner 启动耗时，Fast Path 不再写被跳过的慢步骤；Trace 面板用用户可读摘要展示 input、skill、tool error 和 canvas policy rejection
