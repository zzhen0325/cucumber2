...
- Trace：`run.created` / `input.normalized` 记录服务端重建的 selected nodes、reference nodes、upstream path 和 omitted nodes；启动阶段会用 `run.step.*` 记录上下文重建、输入归一化、计划生成、技能检索、MCP 连接和 Agent Runner 启动耗时；Trace 面板用用户可读摘要展示 input、skill、tool error 和 canvas policy rejection
