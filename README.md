...
- 实现：`server/agent/`
- 编排：Chat Agent、Manager、specialist agents、handoff registry、输入归一化 Agent 和 Runner 按进程缓存；每轮用户上下文仍只来自 per-run `runContext`。Input Normalizer 只负责生成 Task Frame（task、userGoal、routing、inputs、constraints、ambiguities、workflow），`task-router` 负责把 Task Frame 映射到 Agent route；普通短答进入 Chat Agent，单一 specialist 任务直接启动对应 Agent，hybrid / multi-step 任务先进入 Manager，再按 `workflow.requiredAgents` / `workflow.stages` 通过 Agents SDK handoff 委派给 Document Agent、Web Agent、Research Agent 或 Image Agent
- 启动 Fast Path：路由分两层执行。`Quick Router` 只处理确定性 preflight：寒暄、图片生成元信息问题、简单 canvas 操作和已显式归一化的图像模式输入；其余请求进入唯一 LLM Input Normalizer，由一个 `maxTurns=1` 的结构化 Agents SDK agent 输出 Task Frame，再交给 route 映射。LLM normalizer 只接收轻量 upstream 摘要，不接收完整 artifact 正文；重复的同一请求会命中进程内 TTL/LRU Task Frame 缓存。
...
- Agent OS 技能流：兼容 Agent Skills 目录格式；服务端从持久化画布重建可信上下文，非 Chat Agent 任务从 60s 内存缓存检索启用技能的 metadata，首轮只注入 skill cards；画布输入器输入 `/` 可为本轮强制选择一个启用技能，服务端按 id 校验后置顶并预激活。模型必须调用 `activate_skill` 才能读取其它完整 `SKILL.md`，包内资源通过 `read_skill_resource` 按需读取，脚本只能通过 `run_skill_script` 执行
- 路由协议：`input.normalized` 是 Task Frame，以 `task.domain`、`task.action`、`task.intent`、`workflow`、`routing.primaryAgent`、`inputs`、`constraints` 和 `ambiguities` 为准。`workflow` 描述任务图摘要：`mode`、输入模态、目标产物、required capabilities、required agents 和 ordered stages；它不包含最终工具参数，也不直接写画布。`constraints.explicit` 只记录用户明确约束，例如 `output_count`、`dimension`、`aspect_ratio`；各 specialist agent 自己把约束转成最终工具参数。`视觉`、`H5` 等词默认是 domain/context，不会单独触发 Image Agent；明确的 HTML 页面、H5 页面、交互 demo 或 HTML 动画请求归一化为 document/webpage 类任务，并禁止图片生成。
- 通用对话边界：寒暄、普通短答、概念解释、轻量分析、简短总结和基于可信上游上下文的短回答进入 `chat_agent_task`，由 Cucumber Chat Agent 在 Run 节点直接回复，不调用工具也不创建 artifact。提示词/文本改写、knowledge 问答、受限画布操作和复合编排进入 `manager_task`；用户明确要求详细说明、完整规划、长篇方案、报告、文档、模板、提示词模板、可复制/直接使用方案、设定稿或规范时，归一化为 document/markdown artifact 并交给 Document Agent 产出可沉淀的画布文档；搜索、调研分析和引用来源请求交给 Research Agent 产出 research markdown artifact。
...
- Trace：`run.created` / `input.normalized` 记录服务端重建的 selected nodes、reference nodes、upstream path、omitted nodes、route、routerSource、skippedSteps 和归一化结果；启动阶段会用 `run.step.*` 记录实际执行的上下文重建、整理用户需求（内部含快速路由和输入归一化）、计划生成、技能检索和 Agent Runner 启动耗时，tool 结束/失败事件会记录本次工具调用耗时，Trace 面板对旧事件也会从 input/output 时间戳反推工具耗时；Fast Path 不再写被跳过的慢步骤；Trace 面板用用户可读摘要展示 input、skill、tool error 和 canvas policy rejection
...
- 流协议：AI SDK UI `createUIMessageStream` + `data-runtime-event`；普通回答走 `text-*` part，reasoning summary 走 `reasoning-*` part，并同步保留 `agent.message.*` Trace 事件用于刷新回放。图片生成是 Image Agent 本地 function tool；API 启动后会 fire-and-forget 预热模型 provider、Agent world 和技能缓存，打开项目后也会补触发预热，失败只记录日志
...
Agent 模型默认按服务端环境固定优先级选择：Super Relay、Ark、DeepSeek、OpenAI。runtime 使用 Agents SDK 官方 `ModelProvider` + `Runner({ model, modelProvider })` 配置，Manager、specialist 和图片提示词扩写共用同一 provider 解析路径；输入归一化固定使用 Ark。底部输入器提供本轮 Agent 模型选择器，`auto` 沿用服务端优先级，选择具体 provider 时会把 `agentProvider` 写入 `canvasContext` 并只影响本轮 Run；前端不提供自定义模型 ID 或 base URL 输入。底部输入器也提供图片 provider 选择，本轮请求会把 `imageProvider` 写入 `canvasContext`，只影响 `generate_image` 的图片生成 provider。
