...
`TAVILY_API_KEY` is required by the `web.search` tool from `@tavily/ai-sdk`. Web research requests route through Tavily search first, then `document.write` turns the returned sources into a Markdown artifact on the canvas. Missing Tavily credentials are shown directly in the Run node.
...
Every `/api/agent-run` delegates to `server/runtime/executor.ts`. The runtime normalizes input into `AgentInput`, creates an `AgentRun` snapshot, routes obvious non-image executable tasks through deterministic policy before calling the model router, builds a budgeted `BuiltContext`, uses deterministic plans for Markdown document tasks, and executes the plan through generic `runStep` dispatch. For tasks that still need model routing or planning, the selected model provider must return schema-validated `IntentResult` and `PlanStep[]` data. The old `server/run-kernel.ts` remains as a compatibility contract for legacy tests, but it is no longer the main endpoint path. Its `adaptKernelRunToAgentRun` helper maps legacy `success/error` statuses to `completed/failed` and preserves prompt/run ids, tool call data, artifact refs, canvas patch proposals, and error text in a schema-valid `AgentRun`; the old image-only orchestration is explicitly named `executeLegacyImageAgentRunForTests`.

The current runtime Tool Registry registers `vision.analyzeReferenceImages`, `prompt.expand`, `seedream.generateImage`, `document.write`, `web.search`, `web.read`, `asset.analyzeContext`, `page.generate`, and canvas proposal tools for `canvas.createNode`, `canvas.updateNode`, `canvas.createEdge`, and `canvas.attachArtifact`. Tool implementations live by domain under `server/runtime/tools/`: image generation tools in `image-tools.ts`, Markdown document output in `document-tools.ts`, webpage/page tools in `web-page-tools.ts`, canvas proposal tools in `canvas-tools.ts`, and shared tool ids/versioning in `ids.ts`. Image requests route through `prompt.expand + seedream.generateImage`; if upstream context contains image result nodes, the plan first runs a visible reference-image analysis stage with Ark Responses API. Web research/current-source requests route through `web.search` using `@tavily/ai-sdk`, then `document.write` creates a source-grounded Markdown `doc` artifact. Text-first requests such as analysis, summaries, reports, plans, answers, and capability gap reports route through `document.write`, which asks the text model for Markdown text, derives local title/summary metadata, and creates a Markdown `doc` artifact rendered as a canvas `markdownNode`. Compound page requests that ask for analysis/report content and an HTML/page output are routed as multi-step work: `document.write` first produces source Markdown, then `page.generate` turns prior tool results into a `webpage` artifact rendered as a canvas `webpageNode`; `canvas.createNode` is only added when the user explicitly asks for canvas/node mutation. The latest compatible public skill with `slug === "prompt-expand"` is used when no explicit `prompt.expand` manifest exists. If no such skill exists for an image route, model credentials are missing, routing fails, policy denies execution, schema validation fails, an unregistered tool is referenced, a tool times out, or any tool stage fails, the Run node shows the error and the runtime does not continue to later steps.

Unsupported or not-yet-executable non-image requests no longer fall through to image generation. When a dedicated executor exists, the router exposes only that tool chain; when a requested operational capability is not executable, the router can produce a Markdown capability report through `document.write` rather than claiming the action was completed. Planner validation rejects unknown or non-exposed tools, dependency cycles, missing required tool input, and canvas operations that do not have an allowed producer tool.

The runtime records compatible `agent_run_events`, finer `agent_run_step_events` trace rows, and current snapshots in `agent_runs` / `agent_run_steps` after applying `supabase/migrations/20260608005000_agent_runtime_core.sql`. Trace payloads include normalized input, intent, context selection, plan validation, tool IO with tool definition version, schema digests, duration, and logs, artifact refs, canvas operation proposals, graph patch compatibility events, evaluator output, and errors. `server/runtime/context-builder.ts` owns context selection, budget, skill injection, tool exposure, and the runtime `promptParts` consumed by planner, reasoning, reference-image analysis, and prompt expansion steps; `server/prompts.ts` remains the renderer/helper layer for section formatting and legacy run-kernel prompt compatibility.
