...
客户端只提交 `projectId`、`runNodeId`、prompt、`promptNodeId`、主 `selectedNodeId`、`selectedNodeIds`，图像模式下额外提交白名单 `inputMode=image`、`imageAspectRatio`、`imageResultCount` 和 `imageProvider`，以及可选的本轮 `canvasPatch`。Agent 启动时服务端会先原子写入该 patch，再从持久化 `nodes/edges` 重建 upstream context；不信任客户端上传的节点上下文、artifact 内容、URL 或 `contentRef`。多选引用会过滤掉不可引用的 Run 节点，但保留简单文本输出 Run 作为可继续对话的文本上下文。
