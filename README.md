...
- 启动 Fast Path：路由分两层执行。`Quick Router` 只处理确定性 preflight：寒暄、图片生成元信息问题、简单 canvas 操作和已显式归一化的图像模式输入；其余请求进入唯一 LLM Input Normalizer，由一个 `maxTurns=1` 的结构化 Agents SDK agent 输出 Task Frame，再交给 route 映射。
