...
- 实现：`server/agent/`
- 编排：运行时只启动 `Cucumber Super Agent` 一个执行 Agent；不再有 Chat/Manager/specialist agents、handoff registry、direct specialist lanes、Quick Router 或独立 Input Normalizer。每轮用户上下文仍只来自 per-run `runContext`。
- Task Frame：Super Agent 可按需调用内部工具 `set_task_frame`，自己输出 task/userGoal/routing/inputs/constraints/ambiguities/workflow。runtime 把该结构写入 `input.normalized` Trace，并基于它检索 skill cards、生成可见 run plan；Task Frame 是可观测的结构化意图和检索线索，不是执行工具的硬门禁。`routing.primaryAgent` / `workflow.requiredAgents` 只是兼容能力域标签，不决定运行时启动哪个 Agent。
...
- 文本 artifact 工具：Super Agent 使用 `create_text_artifact` 创建 Markdown/document/diagram/code/webpage typed artifact；`diagram` 实际保存为包含 Mermaid block 的 Markdown/doc artifact，HTML 动画、H5 页面和交互 demo 保存为 `webpage` + `html` artifact；HTML artifact 在画布和 Run 输出中支持预览/源码切换，画布 HTML 容器可通过右上角编辑入口进入 Craft.js 白名单组件编辑器，保存后仍写回同一 artifact content；工具只写 artifact 内容和事件，不直接创建画布节点，结果由 runtime materializer 投影到画布
- 网页工具：Super Agent 使用 `fetch_webpage` 抓取公开 http(s) 页面并创建 webpage artifact；此阶段只做 fetch/read，不做浏览器自动操作、登录态访问或多页面爬取，并拒绝 localhost/private network URL。生成 HTML 页面/动画不走网页抓取工具，而是创建 webpage/html artifact
...
- 调研工具：Super Agent 在 OpenAI Agent provider 下可使用官方 Agents SDK hosted `web_search` 搜索公开互联网信息，也可用 `collect_research_sources` 读取用户提供的公开来源，再用 `create_research_artifact` 创建带 citation metadata 的 research markdown artifact；非 OpenAI provider 下没有明确 URL 或可信上下文时会提示切换 provider 或补充 URL
- Agent OS 技能流：兼容 Agent Skills 目录格式；服务端从持久化画布重建可信上下文。Super Agent 需要技能时调用 `set_task_frame`，服务端从 60s 内存缓存检索启用技能 metadata 并返回 skill cards；画布输入器输入 `/` 可为本轮强制选择一个启用技能，服务端按 id 校验后在 skill cards 中置顶。模型必须调用 `activate_skill` 才能读取完整 `SKILL.md`，包内资源通过 `read_skill_resource` 按需读取，脚本只能通过 `run_skill_script` 执行。
- Task Frame 协议：`input.normalized` 来自 Super Agent 按需调用的 `set_task_frame`，以 `task.domain`、`task.action`、`task.intent`、`workflow`、`routing.primaryAgent`、`inputs`、`constraints` 和 `ambiguities` 为结构化记录。`workflow` 描述任务图摘要：`mode`、输入模态、目标产物、required capabilities、required agents 和 ordered stages；它不包含最终工具参数，也不直接写画布。`constraints.explicit` 只记录用户明确约束或输入器明确选择，例如 `output_count`、`dimension`、`aspect_ratio`；Super Agent 仍要结合用户原文、可信上下文、输入器字段和工具返回自行构造最终工具参数。`视觉`、`H5` 等词默认是 domain/context，不会单独触发图片工具；明确的 HTML 页面、H5 页面、交互 demo 或 HTML 动画请求应优先理解为 document/webpage 类任务，但 Task Frame 不作为工具执行禁令。
- 通用对话边界：寒暄、普通短答、概念解释、轻量分析、简短总结和基于可信上游上下文的短回答仍进入同一个 Super Agent；它应直接回复，不调用工具也不创建 artifact。提示词/文本改写直接输出修改后的文本；用户明确要求详细说明、完整规划、长篇方案、报告、文档、模板、提示词模板、可复制/直接使用方案、设定稿或规范时，Super Agent 应使用 `create_text_artifact` 产出可沉淀的画布文档；搜索、调研分析和引用来源请求使用 research/web/knowledge 工具并按需创建 research markdown artifact。
- 画布变更：Agent 只能提出 `CanvasOperation`，由 runtime policy 校验后投影到画布
- 长任务状态：复杂任务、图片任务、重试运行或非空动态计划会写入 task-specific `run.plan.created`，Run 节点展示对应 todo 和当前步骤；简单短任务不创建固定计划骨架。agent、skill、tool 和步骤事件会持续物化到项目快照，用户回到项目后可看到最新状态
- 失败步骤重试：失败工具/脚本步骤可从 Run 节点重试；服务端从旧 Run Trace 重建失败步骤上下文，新 Run 分支保留原失败节点并尽量从失败点继续
- Trace：`run.created` 记录服务端重建的 selected nodes、reference nodes、upstream path 和 omitted nodes；如果 Super Agent 调用 `set_task_frame`，`input.normalized` 会记录该 Task Frame，并标记 `source=set_task_frame`。启动阶段只记录上下文重建和 Agent Runner 启动耗时；计划生成、技能检索、tool 结束/失败事件随着 Super Agent 的工具调用实时写入 Trace。Trace 面板用用户可读摘要展示 input、skill、tool error 和 canvas policy rejection。
- 物化：artifact、canvas operation 和终态事件会幂等写回项目快照；`input.normalized` 只记录 Task Frame/Trace，不预置 pending 结果节点；同一 artifact id 只保留一个结果节点
...
- 流协议：AI SDK UI `createUIMessageStream` + `data-runtime-event`；普通回答走 `text-*` part，reasoning summary 走 `reasoning-*` part，并同步保留 `agent.message.*` Trace 事件用于刷新回放。图片生成是 Super Agent 本地 function tool；API 启动后会 fire-and-forget 预热模型 provider、Agent world 和技能缓存，打开项目后也会补触发预热，失败只记录日志
...
Agent 模型默认按服务端环境固定优先级选择：Super Relay、Ark、DeepSeek、OpenAI。runtime 使用 Agents SDK 官方 `ModelProvider` + `Runner({ model, modelProvider })` 配置，Super Agent 和图片提示词扩写共用同一 provider 解析路径。底部输入器提供本轮 Agent 模型选择器，`auto` 沿用服务端优先级，选择具体 provider 时会把 `agentProvider` 写入 `canvasContext` 并只影响本轮 Run；前端不提供自定义模型 ID 或 base URL 输入。底部输入器也提供图片 provider 选择，本轮请求会把 `imageProvider` 写入 `canvasContext`，只影响 `generate_image` 的图片生成 provider。
...
- Super Relay：`SUPER_RELAY_API_KEY`，可选 `SUPER_RELAY_MODEL`、`SUPER_RELAY_BASE_URL`；默认模型 `opensource/glm5.2`，默认 base URL `https://super-relay.byted.org/v1`
...
内置 `visual-prompt-cookbook` 基于 `server/agent/skills/builtin/visual-prompt-cookbook` 中的 68 个 `style.json` 和 136 张预览图。图片任务统一由 Super Agent 选择图片工具：简单生图可直接调用 `generate_image`；需要特定风格系统、prompt 扩写或复杂参考时，再按需激活绑定 `render_visual_style_prompt` 或 `expand_image_prompt` 的技能。用户上传的 skill 只要绑定同一工具并提供 `references/styles/<slug>/style.json` 或 `styles/<slug>/style.json`，也可走同一机制。旧 `imagegen-prompt-expander` 保留为普通扩写技能。
