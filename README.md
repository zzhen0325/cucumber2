...
- 实现：`server/agent/`
- 编排：Manager、specialist agents、handoff registry、输入归一化 Agent 和 Runner 按进程缓存；每轮用户上下文仍只来自 per-run `runContext`。runtime 先按 artifact-first task protocol 确定路由，单一 specialist 任务直接启动对应 Agent，复合任务由 Manager 通过 Agents SDK handoff 委派给 Document Agent、Web Agent、Research Agent 或 Image Agent
- 启动 Fast Path：`Quick Router` 先用本地规则区分 `smalltalk`、`simple_chat`、`simple_canvas`、`image_task` 和 `complex_agent_task`。寒暄直接回复，简单聊天走轻量 chat agent，确定性的简单画布操作直接经过 canvas policy；无上游引用的简单生图请求走只挂载 `generate_image` 的 Image Fast Agent，不检索技能或连接额外工具；只有复杂或低置信任务才进入完整 Agent Runner 与模型归一化
- 失败 Run 节点显示重试按钮；重试会保留原失败节点，并用原 prompt 与原上游锚点创建新的可见 Agent Run 分支
- 图片工具：`render_visual_style_prompt` 使用已激活 visual style-library 技能的 `style.json` 生成结构化图片提示词；`visual-prompt-cookbook` 是内置实例；`expand_image_prompt` 只作为普通扩写后备；`generate_image` 调用配置的图片 provider（Seedream、Coze 或 ByteArtist）生成图片，`image_matting` 基于选中/上游图片生成抠图素材，`decompose_image` 和 `analyze_media` 产出 Markdown 图片拆解/理解 artifact，`upscale_image` 调用 Seedream 智能超清
...
- Agent OS 技能流：兼容 Agent Skills 目录格式；服务端从持久化画布重建可信上下文，复杂任务从 60s 内存缓存检索启用技能的 metadata，首轮只注入 skill cards；应用内技能 CRUD/import 会立即失效缓存。模型必须调用 `activate_skill` 才能读取完整 `SKILL.md`，包内资源通过 `read_skill_resource` 按需读取，脚本只能通过 `run_skill_script` 执行
...
- 通用对话边界：Manager 直接处理寒暄、短问答、概念解释、轻量分析和简短总结；用户明确要求详细说明、完整规划、长篇方案、调研分析、报告或文档时，归一化为 document/markdown artifact 并交给 Document Agent 产出可沉淀的画布文档。
...
- Trace：`run.created` / `input.normalized` 记录服务端重建的 selected nodes、reference nodes、upstream path、omitted nodes、route、routerSource、cacheHit 和 skippedSteps；启动阶段会用 `run.step.*` 记录实际执行的上下文重建、输入归一化、计划生成、技能检索和 Agent Runner 启动耗时，Fast Path 不再写被跳过的慢步骤；Trace 面板用用户可读摘要展示 input、skill、tool error 和 canvas policy rejection
...
客户端只提交 `projectId`、`runNodeId`、prompt、`promptNodeId`、主 `selectedNodeId`、`selectedNodeIds`，以及可选的本轮 `canvasPatch`。Agent 启动时服务端会先原子写入该 patch，再从持久化 `nodes/edges` 重建 upstream context；不信任客户端上传的节点上下文、artifact 内容、URL 或 `contentRef`。多选引用会过滤掉不可引用的 Run 节点，但保留简单文本输出 Run 作为可继续对话的文本上下文。
...
媒体 provider 独立于 Agent 模型 provider。图片生成支持 `IMAGE_PROVIDER=seedream`、`IMAGE_PROVIDER=coze` 或 `IMAGE_PROVIDER=byteartist`；`image_matting` 通过独立的 `IMAGE_MATTING_PROVIDER=byteartist` provider 接口调用 ByteArtist `image_matting_lemo`；`decompose_image` / `analyze_media` 创建 Markdown artifact，不调用图片 provider；`upscale_image` 仍只支持 Seedream，不支持其他图片 provider 的静默兜底。视频 provider 通过 `VIDEO_PROVIDER` 暴露到 `/api/health`，但当前还没有 `generate_video` tool、video artifact 或画布投影链路。
...
图片生成默认使用 Seedream，需要 `IMAGE_PROVIDER=seedream`、`SEEDREAM_ACCESS_KEY_ID` 和 `SEEDREAM_SECRET_ACCESS_KEY`。也可以使用 Coze：`IMAGE_PROVIDER=coze`、`COZE_IMAGE_TOKEN`，可选 `COZE_IMAGE_URL`、`COZE_IMAGE_SIZE`、`COZE_IMAGE_WATERMARK`、`COZE_IMAGE_MODEL`。ByteArtist/Seed4 使用 `IMAGE_PROVIDER=byteartist`，需要 `BYTEARTIST_BASE_URL`、`BYTEARTIST_AID`、`BYTEARTIST_APP_KEY` 和 `BYTEARTIST_APP_SECRET`，也兼容 `docs/ByteArtist.md` 里的 `GATEWAY_BASE_URL`、`BYTEDANCE_AID`、`BYTEDANCE_APP_KEY`、`BYTEDANCE_APP_SECRET`；默认模型是 `BYTEARTIST_MODEL=seed4_0407_lemo`，也可用通用 `IMAGE_MODEL` 覆盖。Coze 请求体保持 `prompt`、`reference_images`、`size`、`watermark`、`model` 结构；ByteArtist 使用异步 submit/poll，`seed4_0407_lemo` 走 `Prompt` 字段，其他模型默认走 `string` 字段。用户输入提到 `lemo`/`Lemo` 时，服务端会固定使用 ByteArtist `seed4_0407_lemo`；该模型不接收参考图，所以若画布有上游参考图，服务端会先用视觉模型把参考图转成文字描述，再和用户需求整理成最终生图 prompt，之后才调用 ByteArtist。参考图描述默认复用 `ARK_API_KEY`/`ARK_MODEL`，其次 `OPENAI_API_KEY`/`OPENAI_MODEL`，也可通过 `IMAGE_REFERENCE_DESCRIPTION_API_KEY`、`IMAGE_REFERENCE_DESCRIPTION_BASE_URL` 和 `IMAGE_REFERENCE_DESCRIPTION_MODEL` 单独配置。
...
- `bindings.tools` 必须匹配服务端 Tool Registry；`bindings.scopes` 可显式声明权限范围，未声明时会从绑定工具自动推导，例如 `read.skill`、`run.script`、`propose.canvas`、`tool.image.generate`、`tool.image.matting`、`tool.image.decompose`、`tool.media.analyze`、`tool.image.upscale`
