...
const workspaceButtonClassName =
  "!inline-flex !h-9 !min-w-0 !items-center !gap-2 !rounded-cuc-control !px-[12.5px] !text-sm !font-normal !leading-[22px]";
const primaryWorkspaceButtonClassName = cn(
  workspaceButtonClassName,
  "!border-0 !bg-cuc-ink !text-cuc-surface hover:!bg-cuc-ink-hover disabled:!cursor-default disabled:!opacity-[0.42]"
);
const secondaryWorkspaceButtonClassName = cn(
  workspaceButtonClassName,
  "!border-[0.5px] !border-cuc-border !bg-cuc-surface !text-cuc-text-heading hover:!bg-cuc-surface-warm disabled:!cursor-default disabled:!opacity-[0.42]"
);
    ...
    <div className={cn(pageClassName, className)}>
      <div className="mx-auto w-[min(1180px,calc(100vw_-_108px))] max-[760px]:w-full">
        ...
        {(error || status) && (
          <div
            className={cn(
              "mb-3 rounded-cuc-card border-[0.5px] px-2.5 py-2 text-xs leading-4",
              error
                ? "border-cuc-danger-border bg-cuc-danger-surface text-cuc-danger-strong"
                : "border-black/20 bg-cuc-ink/8 text-cuc-ink"
            )}
          >
            {error ?? status}
          </div>
        ...
        <section className="min-w-0 rounded-cuc-card border-[0.5px] border-cuc-border bg-cuc-surface p-2 shadow-none">
          <div className="flex h-cuc-control items-center justify-between py-0 pl-1.5 pr-1 text-[11px] leading-[14px] text-cuc-text-muted">
            <span>
              已配置 {skills.length}
            </span>
          ...
          <div className="grid gap-1">
            {skills.map((skill) => (
              <button
                ...
                className={cn(
                  "grid w-full cursor-pointer gap-1 rounded-cuc-card border-[0.5px] border-transparent bg-transparent p-2 text-left text-cuc-text transition-[background,border-color] duration-[140ms] hover:border-cuc-node-border-hover hover:bg-cuc-surface-warm",
                  skill.id === selectedId && !isNew
                    ? "border-black/40 bg-cuc-ink/8"
                    : ""
                )}
              >
                <div className="flex min-w-0 items-center justify-between gap-2">
                  <span className="truncate text-sm font-medium text-cuc-text">
                    {skill.name}
                  </span>
                ...
                <p className="mt-1 line-clamp-2 text-xs leading-5 text-cuc-text-muted">
                  {skill.description}
                </p>
                <div className="mt-2 flex items-center justify-between text-[11px] text-cuc-text-muted">
            ...
            {!loading && skills.length === 0 && (
              <div className="grid min-h-[108px] place-items-center rounded-cuc-card border-[0.5px] border-dashed border-cuc-preview-border text-center text-cuc-text-muted">
                <FileArchive className="mx-auto mb-2 size-5 text-cuc-text-muted" />
                <p className="text-xs text-cuc-text-muted">还没有技能</p>
              </div>
            )}
          </div>
        </section>

        <section className="min-w-0 overflow-hidden rounded-cuc-card border-[0.5px] border-cuc-border bg-cuc-surface shadow-none">
          <div className="flex items-center justify-between gap-3 border-b-[0.5px] border-cuc-border p-3 max-[760px]:flex-col max-[760px]:items-start">
            <div className="min-w-0">
              <div className="flex min-w-0 items-center gap-2">
                <Sparkles className="size-4 shrink-0 text-cuc-primary" />
                <h2 className="truncate text-sm font-medium text-cuc-text">
                  {isNew
                    ? "新技能"
                    : selectedSummary?.name ?? selectedSkill?.name ?? "技能详情"}
                </h2>
              </div>
              <p className="mt-1 text-xs text-cuc-text-muted">
                {isNew
                  ? "保存后可被 Agent OS 检索"
            ...
            <div className="flex flex-none flex-wrap items-center justify-end gap-1.5 max-[760px]:w-full max-[760px]:justify-start">
              <label className="inline-flex h-cuc-control items-center gap-1.5 rounded-cuc-control border-[0.5px] border-cuc-border bg-cuc-surface px-[9px] text-xs leading-4 text-cuc-text-heading">
                <input
                  type="checkbox"
                  ...
                  className="size-3.5 accent-cuc-primary"
                />
                {updatingEnabled && (
              ...
              {!isNew && selectedSkill && (
                <Button
                  ...
                  variant="ghost"
                  size="icon-sm"
                  className="text-cuc-danger-strong hover:bg-cuc-danger-surface hover:text-cuc-danger-strong"
                  disabled={deleting}
          ...
          <div className="grid gap-3 p-3">
            <div className="flex flex-wrap items-center gap-2 text-xs text-cuc-text-muted">
            ...
            {selectedSkill && (
              <div className="grid grid-cols-2 gap-2 rounded-cuc-card border-[0.5px] border-cuc-border bg-cuc-canvas p-2.5 text-[11px] leading-[14px] text-cuc-text-muted max-[760px]:grid-cols-1">
            ...
            <div className="grid grid-cols-[minmax(0,1fr)_360px] gap-3 max-[760px]:grid-cols-1">
              <div className="min-w-0">
                <div className="mb-2 flex items-center justify-between gap-2">
                  <div className="flex min-w-0 items-center gap-2">
                    <FileText className="size-4 shrink-0 text-cuc-text-muted" />
                    <span className="truncate text-xs font-medium text-cuc-text">
                      SKILL.md
                    </span>
                  </div>
                ...
                <Textarea
                  ...
                  spellCheck={false}
                  className="min-h-[560px] resize-y rounded-cuc-card border-cuc-border bg-cuc-surface font-mono !text-[11px] !leading-[18px]"
                  aria-label="SKILL.md"
                />
              </div>

              <div className="min-w-0 rounded-cuc-card border-[0.5px] border-cuc-border bg-cuc-surface-subtle">
                <div className="border-b-[0.5px] border-cuc-border px-3 py-2.5">
                  <div className="flex items-center justify-between gap-2">
                    <div className="flex min-w-0 items-center gap-2">
                      <FileArchive className="size-4 shrink-0 text-cuc-text-muted" />
                      <span className="truncate text-xs font-medium text-cuc-text">
                        包内资源
                      </span>
                    </div>
                  ...
                  <div className="mt-2 grid grid-cols-4 gap-1 text-[11px] text-cuc-text-muted">
                ...
                <div className="grid max-h-[680px] min-h-[560px] grid-rows-[220px_minmax(0,1fr)]">
                  <div className="overflow-auto border-b-[0.5px] border-cuc-border p-2">
                    {resources.length > 0 ? (
                          ...
                          <button
                            ...
                            className={cn(
                              ...
                              resource.path === selectedResourcePath
                                ? "border-cuc-primary-border-active bg-cuc-primary-surface shadow-cuc-primary-subtle"
                                : "border-transparent hover:border-cuc-primary-border hover:bg-cuc-surface-warm"
                            )}
                          >
                            <ResourceIcon resource={resource} />
                            <span className="min-w-0 flex-1">
                              <span className="block truncate text-xs text-cuc-text">
                                {resource.path}
                              </span>
                              <span className="mt-0.5 flex items-center gap-1 text-[11px] text-cuc-text-muted">
                      ...
                      <div className="flex h-full items-center justify-center rounded-cuc-card border-[0.5px] border-dashed border-cuc-preview-border px-3 text-center text-xs text-cuc-text-muted">
...
function MetaRow({ label, value }: { label: string; value?: string }) {
  return (
    <div className="min-w-0">
      <span className="block text-[11px] uppercase tracking-normal text-cuc-text-muted/80">
        {label}
      </span>
      <strong className="mt-0.5 block truncate font-normal text-cuc-text" title={value}>
        {value || "无"}
      </strong>
    </div>
  );
...
function ResourceStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="min-w-0 rounded-cuc-canvas bg-cuc-surface-code/60 px-2 py-1">
      <span className="block truncate">{label}</span>
      <strong className="block font-medium text-cuc-text">{value}</strong>
    </div>
  );
}
...
function ResourceIcon({ resource }: { resource: AgentSkillResourceSummary }) {
  if (isImageResource(resource.path)) {
    return <ImageIcon className="mt-0.5 size-3.5 shrink-0 text-cuc-text-muted" />;
  }
  if (resource.type === "script") {
    return <Code2 className="mt-0.5 size-3.5 shrink-0 text-cuc-text-muted" />;
  }
  return <FileText className="mt-0.5 size-3.5 shrink-0 text-cuc-text-muted" />;
}
...
function ResourcePreview({
  ...
  if (!resource || !skillId) {
    return (
      <div className="flex items-center justify-center p-4 text-center text-xs text-cuc-text-muted">
        选择资源查看内容
      </div>
    );
  }
  ...
  if (isImageResource(resource.path)) {
    return (
      <div className="min-h-0 overflow-auto p-3">
        <div className="mb-2 truncate text-xs font-medium text-cuc-text">
          {resource.path}
        </div>
        <img
          src={getAgentSkillResourceContentUrl(skillId, resource.path)}
          alt={resource.path}
          className="max-h-[380px] w-full rounded-cuc-card border border-cuc-border object-contain"
        />
      </div>
    );
  }
  ...
  if (!resource.readable) {
    return (
      <div className="flex min-h-0 items-center justify-center p-4 text-center text-xs text-cuc-text-muted">
        该资源可下载，当前不内联预览
      </div>
    );
  }
  ...
  return (
    <div className="min-h-0 overflow-hidden">
      <div className="flex items-center justify-between gap-2 border-b border-cuc-border px-3 py-2">
        <span className="truncate text-xs font-medium text-cuc-text">
          {resource.path}
        </span>
        {loading && <LoadingIndicator ariaLabel="读取资源中" size={14} />}
      </div>
      <pre className="h-full overflow-auto whitespace-pre-wrap break-words p-3 font-mono text-[11px] leading-5 text-cuc-text">
