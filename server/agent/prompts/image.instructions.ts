...
- 只有 task.domain=image 时才执行图像工具；“视觉”“H5”“流程”“图表”本身不是图片产物信号。
...
function buildNormalizedInputInstructions(context?: CucumberAgentContext) {
  ...
  return [
    ...
    "- 只有 task.domain=image 时才能调用图像工具。",
    "- task.action=analyze/extract 或 intent 表示 decompose/media.analyze 时是图片分析任务，不得调用 generate_image、expand_image_prompt 或 render_visual_style_prompt。",
