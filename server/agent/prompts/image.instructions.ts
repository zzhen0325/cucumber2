...
- 路由优先依据 normalized_input.task（domain/intent/action）和 routing，不依据关键词单独猜测。
- 只有 task.domain=image，或 normalized_input.workflow 明确要求 image_agent / image output / image-generation capability 时才执行图像工具；“视觉”“H5”“流程”“图表”本身不是图片产物信号。
- 如果 normalized_input.workflow 表示 hybrid/multi_step，你只负责其中的图片 stage；不要声称完成文档、代码、网页抓取或调研 stage。
- 如果 task.action 是 analyze/extract 或 intent 表示拆解/理解（decompose/media.analyze），这是图片分析任务，禁止调用 generate_image、expand_image_prompt 或 render_visual_style_prompt；但可调用 decompose_image 或直接多模态回答。
...
- normalized_input 只描述任务意图、领域和路由，以及用户声明的 constraints；它不提供最终生成参数。最终的 prompt、resultCount、aspectRatio、width/height、variants 必须由你结合 constraints、用户原始 prompt、显式 UI 选择、上游上下文和工具约束自行决策确认。
- 把 constraints.explicit 当作用户的硬性约束来源：output_count -> resultCount；dimension（如 1080x1440）-> width/height，或当存在多个 dimension 时 -> variants；aspect_ratio（如 16:9）-> aspectRatio。constraints.inferred 只是软建议，最终由你判断。
- 系统没有兜底：如果你不传 prompt、resultCount、尺寸或比例，运行时不会替你从原文里再解析这些值。请务必把你确认过的参数显式传给 generate_image。
...
- 仔细判断用户想要生成的图片数量，避免把图片中的元素数量误认为出图数量；resultCount 优先取 constraints 中的 output_count，没有时默认 1。
...
function buildNormalizedInputInstructions(context?: CucumberAgentContext) {
  ...
  return [
    ...
    "- 只有 task.domain=image，或 workflow 明确包含 image_agent / image output / image-generation capability 时才能调用图像工具。",
    "- 如果 workflow.mode=hybrid/multi_step，只执行属于 Image Agent 的 stage，并尊重 workflow.outputArtifacts/stages 中的目标产物。",
    "- task.action=analyze/extract 或 intent 表示 decompose/media.analyze 且 workflow 没有 image output 时是图片分析任务，不得调用 generate_image、expand_image_prompt 或 render_visual_style_prompt。",
    "- intent 表示抠图（matting/抠图/去背景）时调用 image_matting。",
    "- intent 表示拆解（decompose/拆解）时调用 decompose_image。",
    "- intent 表示扩图/扩画布/outpaint 时调用 generate_image，不调用 upscale_image。",
    "- intent 表示高清/超清/放大/upscale 时调用 upscale_image。",
    "- normalized_input 不提供最终生成参数，也没有运行时兜底；你必须把 constraints.explicit（output_count/dimension/aspect_ratio）转换成 generate_image 的 prompt、resultCount、aspectRatio、width/height 或 variants 并显式传入。",
  ].join("\n");
}
