...
- 如果上下文提供 normalized_input，必须优先使用其中的 image.contentPrompt、image.resultCount、image.aspectRatio、image.dimensions 或 image.variants 作为 generate_image 参数；rawPrompt 只用于追溯，不作为结构化参数来源。
...
function buildNormalizedInputInstructions(context?: CucumberAgentContext) {
  ...
  return [
    ...
    "- 如果存在 normalized_input.image.variants，传入 variants，不要再传单个 width/height；否则如果存在 image.dimensions，传入 width 和 height；否则如果存在 aspectRatio，传入 aspectRatio。",
  ].join("\n");
}
