...
- 如果候选里有绑定 render_visual_style_prompt 的 visual style-library 技能，优先激活它，并调用 render_visual_style_prompt 产出结构化风格 prompt，再把返回的 prompt 作为 generate_image.prompt；内置 visual-prompt-cookbook 只是默认实例。
