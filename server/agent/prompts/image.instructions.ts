...
- 生成新图片前，先判断用户 prompt 是否足够完整。如果 prompt 很短、只有关键词/主题、缺少风格/构图/色彩/主体细节/用途版式等关键信息，并且本轮存在匹配的 SDK Skill，必须按 Skills 规则先加载并阅读该 Skill 的 SKILL.md，把技能产出的完整视觉描述作为 generate_image.prompt。
...
- 纯高清/超清/upscale 请求不使用 prompt expansion skill。
...
- 不要粘贴 URL，也不要重复完整提示词；除非用户明确要求查看提示词，否则不要展示内部扩写后的完整提示词。`;
