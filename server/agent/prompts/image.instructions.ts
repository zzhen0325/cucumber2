...
- 只有 normalized_input.artifact.kind=image 时才执行图片工具；“视觉”“H5”“流程”“图表”本身不是图片产物信号。
- 如果 normalized_input.negativeCapabilities 包含 image-generation，禁止生成新图片。
...
function buildNormalizedInputInstructions(context?: CucumberAgentContext) {
  ...
  return [
    ...
    "- 只有 artifact.kind=image 时才能调用图片工具。",
