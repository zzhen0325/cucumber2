...
- 如果 normalized_input.workflow 表示 hybrid/multi_step，你只负责其中的 document/code/webpage/diagram 文本产物 stage；不要声称完成图片生成、网页抓取或调研 stage。
- 当 normalized_input.task.intent 表示 diagram/mermaid/flowchart/sequence 时，你负责创建 Markdown/Mermaid 图表 artifact，不要把它当成图片生成任务。
- 当 normalized_input.task.intent 表示 webpage/html/H5/demo/animation 时，你负责创建完整 HTML artifact；HTML 动画、H5 页面和交互 demo 都属于文本产物，不要把它当成图片生成任务。
...
function buildSkillInstructions(context?: CucumberAgentContext) {
  ...
  return [
    ...
    activated.length
      ...
      : "使用技能前必须先 activate_skill；只激活与当前 Task Frame 的 domain、intent、action 和 constraints 匹配的技能。若技能说明提到 references/scripts/assets，先用 read_skill_resource 查看当前任务需要的文本资源。",
  ]
    .filter(Boolean)
    .join("\n");
}
...
function buildNormalizedInputInstructions(context?: CucumberAgentContext) {
  ...
  return [
    ...
    "- 如果 workflow.mode=hybrid/multi_step，只执行属于 Document Agent 的 stage，并尊重 workflow.outputArtifacts/stages 中的目标产物。",
    "- document/markdown/diagram/code/webpage/html 类任务必须产出 create_text_artifact。",
    ...
    "- constraints.explicit 是用户明确约束；最终 title、content、format、language 必须由你结合 rawPrompt、Task Frame 和可信 upstream context 自己构造。",
  ].join("\n");
}
