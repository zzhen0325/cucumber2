...
- 新增 specialist agent registry，集中声明 specialist name、enabled routes、required tools、produced artifact types 和 handoff policy；Manager 通过 registry 生成 Agents SDK handoff，specialist model 仍由 runtime 统一注入。
...
- 新增 Cucumber Document Agent：负责 Markdown/document/diagram/code/webpage 文本类 artifact 生成和改写；document/webpage route 由 runtime 直接启动或在复合任务中经 Manager handoff，不由 Manager 直接创建内容 artifact。
...
- 新增 Cucumber Web Agent：负责公开 http(s) 网页 fetch/read；web route 由 runtime 直接启动或在复合任务中经 Manager handoff，Manager 不直接抓取网页。
...
- 新增 Cucumber Research Agent：负责 source-based research；research route 由 runtime 直接启动或在复合任务中经 Manager handoff，但没有明确公开 URL 或可信来源时要求用户补充来源，不做通用 web search。
...
- `generate_image` 工具只做 Agent tool 边界、artifact 事件和图片 provider 调用编排；`image_matting` 工具只依赖 `runImageMatting` 统一接口，默认用 `IMAGE_MATTING_PROVIDER=byteartist` 调用 ByteArtist `image_matting_lemo`，请求参数默认为 `blue=-1`、`green=-1`、`only_mask=0`、`red=-1`、`refine_mask=2`，源图优先由服务端签发 R2 read URL，并通过智创网关标准表单字段 `source` 传入；只有没有 provider 可拉取 URL 但已有服务端 bytes 时才使用 `base64file`；图片生成当前默认走 Seedream 5（ByteArtist `seed5_duotu_zz`），前端/API 只开放 `imageProvider=seed5_duotu_zz` 和 `imageProvider=byteartist`（Lemo），Seedream 4.6 与 Coze Image 暂时不在白名单内。其中 ByteArtist 使用统一签名表单提交 `submit_task_v2` 并轮询 `batch_get_result_v2`，模型差异封装在 provider adapter 中，当前 `seed4_0407_lemo` 使用 `Prompt` 字段且标记为 text-only，`seed5_duotu_zz` 使用 `extra_inputs.height/width` 和 `user_prompt`，默认 2048x2048，并通过 multipart `files[]` + `input_img_type=multiple_files` 上传最多 6 张参考图；Coze 请求体 adapter 暂时保留为 `prompt`、`reference_images`、`size`、`watermark`、`model`，但不会由客户端本轮 provider 选择进入；`reference_images` 是由上游图片 URL 生成的 `{ url }` file dict 数组或支持参考图的 ByteArtist 模型参考图字段，`size/model` 是字符串，`watermark` 是布尔值，空配置不发送占位字段。
...
- `generate-image.request.ts` 在 Seedream/ByteArtist 生图请求发出前统一归一化尺寸：显式 `width/height`、variants 或比例推导出的尺寸如果短边小于 1024px，会按原比例放大到短边至少 1024px，再进入 provider 请求。
...
- 上游图片引用对 Agent prompt 仍隐藏真实 URL；调用 Seedream/Coze/ByteArtist 前，服务端仅根据 `r2://agent-assets/...` 临时签发 provider 可读 URL。
- Agent 模型 provider 使用 Agents SDK 官方 `ModelProvider` + `Runner({ model, modelProvider })` 写法；默认优先级为 Super Relay、Ark、DeepSeek、OpenAI。前端输入器可为本轮 Run 选择 `agentProvider`，`auto` 沿用服务端优先级；Super Agent 和 prompt expansion 使用同一本轮 provider。
